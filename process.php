...
if (isset($_FILES['csv_file']) && $_FILES['csv_file']['error'] == 0) {
    ...
    if (($handle = fopen($fileTmpPath, "r")) !== FALSE) {
        ...
        while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
            ...
            if (array_key_exists($librarySection, $stats)) {
                ...
                if (isset($stats[$librarySection]['q1'][$q1]))
                    $stats[$librarySection]['q1'][$q1]++;
                if (isset($stats[$librarySection]['q2'][$q2]))
                    $stats[$librarySection]['q2'][$q2]++;
                if (isset($stats[$librarySection]['q3'][$q3]))
                    $stats[$librarySection]['q3'][$q3]++;
                if (isset($stats[$librarySection]['q4'][$q4]))
                    $stats[$librarySection]['q4'][$q4]++;

                if (strcasecmp($sat, 'Yes') == 0)
                    $stats[$librarySection]['sat']['Yes']++;
                elseif (strcasecmp($sat, 'No') == 0)
                    $stats[$librarySection]['sat']['No']++;

                // Case-insensitive matching for Overall
                foreach ($stats[$librarySection]['overall'] as $key => $val) {
                    if (strcasecmp($overall, $key) == 0)
                        $stats[$librarySection]['overall'][$key]++;
                }
    ...
    if (!file_exists($templatePath)) {
        die("Error: master_template.xlsx is missing!");
    }
    ...
    foreach ($departments as $dept => $config) {
        ...
        if (!$sheet)
            continue; // Safety check if tab is missing
    ...
    if ($commentsSheet) {
        ...
        foreach ($globalRecommendations as $rec) {
            if ($rRow > 51)
                break; // Hard limit based on your mapping
    ...
    // 4. SAVE FILE, UPDATE LEDGER, AND EXPORT JSON
    $recordId = uniqid('rep_');
    $fileName = 'EVAL_REPORT_' . str_replace(' ', '_', $reportDate) . '_' . $recordId . '.xlsx';

    // --- PHASE 1: Folder Segregation (by Year) ---
    $exportYear = isset($jsonYear) ? $jsonYear : date('Y');
    $saveDir = __DIR__ . '/exports/' . $exportYear . '/';
    ...
    $downloadUrl = 'exports/' . $exportYear . '/' . $fileName;

    // Save the physical Excel file
