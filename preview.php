...
<body class="bg-gray-50 min-h-screen font-sans antialiased flex flex-col">
    ...
    <header class="bg-acad-blue text-white shadow-md border-b-4 border-acad-gold">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex items-center justify-between">
            ...
            <div class="flex space-x-4">
                ...
                <button id="btn-export-excel" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-acad-gold hover:bg-yellow-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-acad-gold transition-colors shadow-sm hidden">
    ...
    <main class="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 w-full">
        ...
        <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200 mb-8 w-full max-w-7xl">
            ...
            <div class="flex flex-col sm:flex-row gap-6">
                <!-- Year Filter -->
                <div class="flex-1">
                    <label for="filter-year" class="block text-sm font-medium text-gray-700 mb-1">1. Filter by Year</label>
                    <div class="relative">
                        <select id="filter-year" class="block w-full pl-3 pr-10 py-2.5 text-base border-gray-300 focus:outline-none focus:ring-acad-blue focus:border-acad-blue sm:text-sm rounded-md border appearance-none text-gray-900 leading-5">
                ...
                <!-- Month Filter -->
                <div class="flex-1">
                    <label for="filter-month" class="block text-sm font-medium text-gray-700 mb-1">2. Filter by Month</label>
                    <div class="relative">
                        <select id="filter-month" disabled class="block w-full pl-3 pr-10 py-2.5 text-base border-gray-300 focus:outline-none focus:ring-acad-blue focus:border-acad-blue sm:text-sm rounded-md border appearance-none text-gray-900 leading-5 bg-gray-100 disabled:opacity-50">
                ...
                <!-- Branch Filter -->
                <div class="flex-1">
                    <label for="filter-branch" class="block text-sm font-medium text-gray-700 mb-1">3. Filter by Branch</label>
                    <div class="relative">
                        <select id="filter-branch" disabled class="block w-full pl-3 pr-10 py-2.5 text-base border-gray-300 focus:outline-none focus:ring-acad-blue focus:border-acad-blue sm:text-sm rounded-md border appearance-none text-gray-900 leading-5 bg-gray-100 disabled:opacity-50">
        ...
        <div id="status-message" class="bg-gray-100 text-gray-700 text-center py-10 px-6 rounded-lg border border-gray-300 shadow-sm w-full max-w-7xl mb-8 flex flex-col items-center justify-center">
        ...
        <!-- Dashboard Content -->
        <div id="dashboard-content" class="hidden space-y-8 bg-white p-8 rounded-lg shadow-sm border border-gray-200 text-black w-full max-w-7xl">
            
            <div class="text-center mb-8">
                <h2 class="text-xl font-bold uppercase tracking-wide text-gray-900">SUMMARY OF LIBRARY SERVICE EVALUATION</h2>
                <h3 id="report-date-display" class="text-md font-bold italic uppercase mt-1 text-gray-800">DECEMBER 2022</h3>
            </div>
            
            <!-- Dashboard Visuals -->
            <div id="dashboard-visuals" class="mb-10 grid grid-cols-1 lg:grid-cols-3 gap-6 hidden">
                <div class="space-y-6 lg:col-span-1">
                    <div class="bg-blue-50 p-6 rounded-lg border border-blue-200 shadow-sm flex flex-col items-center justify-center h-32">
                    ...
                    <div class="bg-green-50 p-6 rounded-lg border border-green-200 shadow-sm flex flex-col items-center justify-center h-32">
                        <p class="text-sm font-semibold text-green-800 uppercase text-center mb-2">Overall Satisfaction</p>
                        <p id="kpi-satisfaction" class="text-3xl font-bold text-green-900">-</p>
                    </div>
                    <div class="bg-amber-50 p-6 rounded-lg border border-amber-200 shadow-sm flex flex-col items-center justify-center h-32">
                        <p class="text-sm font-semibold text-amber-800 uppercase text-center mb-2">Overall Rating</p>
                        <p id="kpi-rating" class="text-3xl font-bold text-amber-900">-</p>
                    </div>
                </div>
                <div class="lg:col-span-2 bg-white p-4 rounded-lg border border-gray-200 shadow-sm min-h-[300px] relative">
                    <canvas id="reporting-chart"></canvas>
                </div>
            </div>
            
            <!-- Automated Service Analysis -->
            <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200 mb-10 w-full relative hidden" id="automated-analysis-container">
                <div class="flex items-center justify-between mb-4 border-b border-gray-100 pb-3">
                    <h3 class="text-xl font-bold text-gray-900 flex items-center">
                        <svg class="w-5 h-5 text-acad-gold mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>
                        Automated Service Analysis
                    </h3>
                    <button onclick="copyAnalysisText(this)" class="inline-flex items-center px-3 py-1.5 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-acad-blue transition-colors">
                        <svg class="w-4 h-4 mr-1.5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg>
                        Copy
                    </button>
                </div>
                <p id="analysis-text" class="text-gray-700 text-lg leading-relaxed">
                    Data loaded successfully. The system detects responses across evaluated branches. Please implement an LLM analysis generation endpoint here to populate detailed programmatic service insights based on the selected filters.
                </p>
            </div>
            ...
            <div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full border-collapse border border-black text-sm">
                        <thead>
                            <tr>
                                <th class="border border-black bg-[#fff2cc] px-2 py-3 text-center align-middle font-normal w-32">Sections / Branch<br>Library</th>
                                <th class="border border-black bg-[#fff2cc] px-2 py-3 text-center align-middle font-normal w-24">Respondents</th>
                                <th class="border border-black bg-[#fff2cc] px-2 py-3 text-center align-middle font-normal w-40">The library has<br>sufficient resources<br>for my research and<br>information needs</th>
                                <th class="border border-black bg-[#fff2cc] px-2 py-3 text-center align-middle font-normal w-40">Library staff provided<br>assistance in a timely and<br>helpful manner</th>
                                <th class="border border-black bg-[#fff2cc] px-2 py-3 text-center align-middle font-normal w-40">The process of<br>borrowing, returning<br>and renewal of library<br>resources is</th>
                                <th class="border border-black bg-[#fff2cc] px-2 py-3 text-center align-middle font-normal w-40">The information/procedure<br>provided by the library<br>staff were easy to<br>understand</th>
                                <th class="border border-black bg-[#fff2cc] px-2 py-3 text-center align-middle font-bold">MEAN RATING</th>
                            </tr>
                        </thead>
            ...
            <div class="mt-8">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full max-w-2xl border-collapse border border-black text-sm">
                        <thead>
                            <tr>
                                <th class="border border-black bg-[#fff2cc] px-2 py-3 text-center align-middle font-normal w-48">Sections / Branch<br>Library</th>
                                <th class="border border-black bg-[#fff2cc] px-2 py-3 text-center align-middle font-normal">Are you satisfied with the library<br>service you have received?</th>
                            </tr>
                        </thead>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full max-w-2xl border-collapse border border-black text-sm">
                        <thead>
                            <tr>
                                <th class="border border-black bg-[#fff2cc] px-2 py-3 text-center align-middle font-normal w-48">Sections / Branch<br>Library</th>
                                <th class="border border-black bg-[#fff2cc] px-2 py-3 text-center align-middle font-normal">Overall, how would you rate the library<br>service/s we provide?</th>
                            </tr>
                        </thead>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // --- OPTIONAL: Force automatic download on page load ---
                // We use a temporary link element to trigger it silently in the background
                setTimeout(() => {
                ...
                }, 500); // 500ms delay ensures the dashboard renders first
            ...
            function drawTableRow(bData, branch, isOverall) {
                ...
                tr1.innerHTML = `
                    ...
                    <td class="${centerTdClass}">${p1}</td>
                    <td class="${centerTdClass}">${p2}</td>
                    <td class="${centerTdClass}">${p3}</td>
                    <td class="${centerTdClass}">${p4}</td>
                    <td class="${centerTdClass} font-bold text-[#1e3a8a]">${meanRating}</td>
                `;
                ...
                if (isOverall) {
                    ...
                    tr2.innerHTML = `
                        ...
                        <td class="${centerTdClass} text-[#1e3a8a] font-bold">${sat}</td>
                    `;
                } else {
                    tr2.innerHTML = `
                        <td class="${firstTdClass}">${branch}</td>
                ...
                if (isOverall) {
                    ...
                    tr3.innerHTML = `
                        <td class="${firstTdClass} text-right">Overall Rating:</td>
                        <td class="${centerTdClass} text-[#1e3a8a] font-bold">${over}</td>
                    `;
                } else {
                    tr3.innerHTML = `
                        <td class="${firstTdClass}">${branch}</td>
            ...
            function renderView(subset, selectedBranch = null) {
                ...
                const kpiS = document.getElementById('kpi-satisfaction');
                const kpiO = document.getElementById('kpi-rating');
                document.getElementById('dashboard-visuals').classList.remove('hidden');

                if (!subset || subset.length === 0) {
                    kpiR.textContent = 'N/A';
                    kpiS.textContent = 'N/A';
                    kpiO.textContent = 'N/A';
                ...
                if (!kpiDataObj || Object.keys(kpiDataObj).length === 0) {
                    ...
                    kpiS.textContent = formatNum(kpiDataObj.partIISatisfaction || kpiDataObj.part2, true);
                    kpiO.textContent = formatNum(kpiDataObj.partIIIOverallRating || kpiDataObj.part3, true);
                }
            ...
            function updateChart(subset) {
                ...
                branches.forEach(b => {
                    ...
                    if (row && row.partIIIOverallRating !== '-' && row.partIIIOverallRating) {
                        labels.push(b);
                        dataPoints.push(parseFloat(row.partIIIOverallRating));
                    }
                });
